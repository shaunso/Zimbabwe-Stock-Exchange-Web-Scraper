...
const getREITs = async () => {
  ...
  const result = await page.evaluate( () => {
    ...
    const data = document.querySelectorAll('section.elementor-section:nth-child(8) tr');
    ...
    return Array.from(data).map( ( el, pos ) => {
      if ( pos > 0 ) {      
        const reit = (el.querySelector('td:nth-child(1)').innerText.trim());
        const price = (el.querySelector('td:nth-child(2)').innerText.trim());

        return { currentDate, reit, price };
      } else return
    }).filter( el => el != null)
  })
...
const reits = getREITs().then( value => {
  console.log(value);

  // append the date to the first column of the JSON file
  fs.writeFile(`./reit/json/${value[0].currentDate}.json`, JSON.stringify(value), err => {
    if (err) throw err;
    console.log(`Data successfully saved for ${value[0].currentDate} to JSON`);
  });  

  // append the date to the first column of the CSV file
  fs.appendFileSync('./reit/closing_price.csv', `\n${value[0].currentDate}`, err => {
    if (err) throw err;
    console.log(`Saving closing price data for ${value[0].currentDate} to CSV...`);
  });
  // fs.appendFileSync('./reit/trading_volume.csv', currDate, err => {
  //   if (err) throw err;
  // console.log(`Saving trade volume data for ${value[0].currentDate} to CSV...`);
  // });

  // appending the CSV files with the scrapped data
  for ( let entity of value) {
    fs.appendFileSync('./reit/closing_price.csv', `,${entity.price}`, err => {
      if (err) throw err;
      console.log(`${entity.reit}: ZIG${entity.price}`);
    });
  }
  console.log(`Closing price data for ${value[0].currentDate} successfully saved`)
  // for ( let entity of value) {
  //   fs.appendFileSync('./reit/trading_volume.csv', `,${entity.volume}`, err => {
  //     if (err) throw err;
  //     console.log(`${entity.reit}: ZIG${entity.volume}`);
  //   });
  // }
  // console.log(`Trade volume data for ${value[0].currentDate} successfully saved`)
});

module.exports = reits;